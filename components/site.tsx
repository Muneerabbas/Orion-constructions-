...
const whatsappMaterials = whatsappFor('Hello Orion Constructions, I would like to enquire about construction materials supply.');
const nav = [['Services', '#services'], ['Materials', '#materials'], ['Approach', '#approach'], ['About', '#about'], ['Contact', '#contact']];

function Header() {
  const [open, setOpen] = useState(false);
  return <header className="site-header"><div className="shell header-inner"><a href="#home" className="brand"><span className="brand-logo"><Image src="/assets/logo.jpeg" alt="Orion Constructions logo" width={52} height={52}/></span><span><strong>Orion</strong><small>Constructions · Kashmir</small></span></a><nav className="desktop-nav">{nav.map(([label, href]) => <a key={label} href={href}>{label}</a>)}</nav><div className="header-actions"><a className="phone-link" href={`tel:${phone}`}><Phone size={15}/> +91 60054 13428</a><a className="button button-dark button-small" href={whatsappHeader}>Start a project <ArrowUpRight size={15}/></a></div><button className="menu-button" onClick={() => setOpen(!open)} aria-label="Toggle menu">{open ? <X/> : <Menu/>}</button></div>{open && <nav className="mobile-nav">{nav.map(([label, href]) => <a onClick={() => setOpen(false)} key={label} href={href}>{label}</a>)}<a className="button button-dark" href={whatsappHeader}>Start a project <ArrowUpRight size={15}/></a></nav>}</header>;
}
...
function Materials() {
  return <section id="materials" className="materials section"><div className="shell"><SectionHeading eyebrow="03 / Additional service" title="Construction materials supply">As an additional service, Orion Constructions also supplies essential construction materials across Kashmir for residential, commercial and other construction projects.</SectionHeading><a className="text-link materials-cta" href={whatsappMaterials}>Enquire about material supply <ArrowUpRight size={16}/></a><div className="materials-grid">{materials.map((material) => <a className="material-card" key={material.title} href={material.whatsapp} aria-label={`Enquire about ${material.title}`}><div className="material-photo"><Image src={material.image} alt={material.title} fill sizes="(max-width: 900px) 100vw, 33vw"/></div><div className="material-body"><h3>{material.title}</h3><p>{material.copy}</p></div></a>)}</div></div></section>;
}
...
function Enquiry() {
  ...
  return <section id="contact" className="enquiry section"><div className="shell enquiry-grid"><div><Eyebrow>07 / Let’s talk</Eyebrow><h2>Have a project<br/><em>in mind?</em></h2><p>Tell us a little about what you’re building. We’ll get back to you and take it from there.</p><div className="contact-details"><span><MapPin size={17}/> Across Kashmir</span><a href={`tel:${phone}`}><Phone size={17}/> +91 60054 13428</a><a href={`tel:${alternatePhone}`}><Phone size={17}/> +91 70510 09486</a><a href={`mailto:${email}`}>{email}</a></div></div><form onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; const data = new FormData(form); const message = ['Hello Orion Constructions, I would like to make an enquiry.', `Name: ${data.get('name')}`, `Phone: ${data.get('phone')}`, `Project: ${data.get('project')}`, `Details: ${data.get('details') || 'Not provided'}`].join('\n'); setSent(true); window.location.href = whatsappFor(message); }}><label>Name<input name="name" required placeholder="Your name"/></label><label>Phone number<input name="phone" required type="tel" placeholder="Your phone number"/></label><label className="full">What are you building?<select name="project"><option>House / residential project</option><option>Commercial space</option><option>Civil or structural work</option><option>Renovation</option></select></label><label className="full">A few details<textarea name="details" rows={4} placeholder="Location, size, timeline..."/></label><button className="button button-accent full" type="submit">{sent ? <><Check size={17}/> Opening WhatsApp</> : <>Send enquiry <ArrowUpRight size={17}/></>}</button></form></div></section>;
}

function Footer() { return <footer><div className="shell footer-grid"><div><a className="brand" href="#home"><span className="brand-logo"><Image src="/assets/logo.jpeg" alt="Orion Constructions logo" width={64} height={64}/></span><span><strong>Orion</strong><small>Constructions · Kashmir</small></span></a><p className="footer-note">Building useful, lasting spaces across Kashmir.</p></div><div><Eyebrow>Navigate</Eyebrow>{nav.map(([label, href]) => <a className="footer-link" key={label} href={href}>{label}</a>)}</div><div><Eyebrow>Connect</Eyebrow><a className="footer-link" href={whatsappFooter}><MessageCircle size={15}/> WhatsApp</a><a className="footer-link" href={companyInstagram}><Instagram size={15}/> Company Instagram</a><a className="footer-link" href={founderInstagram}><Instagram size={15}/> Founder Instagram</a><a className="footer-link" href={facebook}>Facebook</a><a className="footer-link" href={`mailto:${email}`}>{email}</a></div></div><div className="shell footer-bottom"><span>© 2026 Orion Constructions</span><span>Construction · Civil works · Kashmir</span></div></footer>; }
